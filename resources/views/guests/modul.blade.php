        ...
        <div class="container">
            <form id="searchForm" action="{{ route('guest.modul') }}" method="GET">
                <div id="inputSearchModul" class="input-group mb-3">
                    <input type="text" class="form-control" id="cariModul" name="cariModul"
                        placeholder="Masukkan Judul atau Tanggal" aria-label="Recipient's username"
            ...
            @if (count($dataModul) >= 1)
              <div class="d-flex flex-wrap justify-content-center align-items-center">
                  @foreach ($dataModul as $m)
                      <x-card-modul gambar="{{ $m->gambar }}" judul="{{ $m->nama }}" deskripsi="{{ $m->deskripsi }}"
                          slug="{{ $m->slug }}" date="{{ $m->created_at->toDateString() }}" />
                  @endforeach
              </div>
            @else
            <p class="fw-bold text-center my-5 fs-3"> Tunggu info modul dari kami ya. </p>
